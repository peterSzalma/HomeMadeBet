let dom = {
    loadMatches: function (status) {
        let url = 'https://api.football-data.org/v2/competitions/PL/matches?status=' + status;
        fetch(url, {
            headers: {
                'X-Auth-Token': 'ad621f63eed54e48b623639a33fb6597'
            },
            dataType: 'json',
            type: 'GET'
        })
    ...
    showLeaderBoard: function (summary) {
    ...
    showScoreBoard: function (summary) {
    ...
    showRemainingMatches: function (summary) {
...
$.ajax({
    ...
    // do something with the response, e.g. isolate the id of a linked resource
    dom.showRemainingMatches(response);
    console.log(response);
});
}
